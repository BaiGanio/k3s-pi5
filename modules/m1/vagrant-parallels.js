...

  {
    ...
    items: [
      ...
      },
      {
        ...
        description: 'Writes the full Vagrantfile to disk in one shot. It declares both VMs (<code>sqlserver</code> first, because <code>bgapi</code> depends on it being reachable), each with its own box, hostname, private-network IP, port forward, and provisioning script. Hostname is set by <code>db.vm.hostname</code> / <code>web.vm.hostname</code> — Vagrant applies it at boot, no manual <code>hostnamectl</code> step required.',
        parts: [
        ...
        example: "$ ls -la\ntotal 8\ndrwxr-xr-x  3 you  staff   96 May 17 12:00 .\ndrwxr-xr-x  5 you  staff  160 May 17 12:00 ..\n-rw-r--r--  1 you  staff  2.4K May 17 12:00 Vagrantfile",
        why: "Front-loading the full Vagrantfile makes the topology obvious from line one: two VMs, each with its own box, hostname, and provisioner. There is no in-between state where one VM is being renamed twice. Every command in the rest of this module is something the provisioner inside this file already runs — the later sections only explain what it does.",
      },
      {
        id: 203,
