...
(function () {
  ...
  let activeModuleGroupName = null;
  ...
  // Track which welcome-screen sections are collapsed. Default: all collapsed,
  // so the overview starts shrunk and the user expands topics on demand.
  const WELCOME_COLLAPSED_KEY = 'welcome-collapsed-groups';
  function getWelcomeCollapsed() {
    const stored = sessionStorage.getItem(WELCOME_COLLAPSED_KEY);
    if (stored === null) return new Set(MODULE_REGISTRY.map(g => g.group));
    try { return new Set(JSON.parse(stored) || []); }
    catch { return new Set(MODULE_REGISTRY.map(g => g.group)); }
  }
  function saveWelcomeCollapsed() {
    sessionStorage.setItem(WELCOME_COLLAPSED_KEY, JSON.stringify([...welcomeCollapsed]));
  }
  const welcomeCollapsed = getWelcomeCollapsed();
  ...
  function isDark() {
    // Deep space is the home dimension — dark by default, light on request
  ...
  function loadModule(mod) {
    ...
    // Look up which group this module belongs to
    activeModuleGroupName = null;
    for (const g of MODULE_REGISTRY) {
      if (g.modules.some(m => m.id === mod.id)) {
        activeModuleGroupName = g.group;
        break;
      }
    }
    ...
    if (!backLink) {
      ...
      backLink.addEventListener('click', (e) => {
        e.preventDefault();
        if (activeModuleGroupName) {
          showWelcomeAndScrollTo(activeModuleGroupName);
        } else {
  ...
  function renderWelcome(view) {
    ...
    greeting.innerHTML = view === 'playground'
      ? `
        <h2>The Garage — hands-on practice labs</h2>
        <p>Guided exercises with real commands. Break things here, not in production.</p>
      `
      : `
        <h2>Portal from zero to Kubernetes on a&nbsp;Raspberry&nbsp;Pi&nbsp;5</h2>
        <p>Pick a module and portal in — every command explained: what it does, why it's there, and what you'll see.</p>
      `;
    ...
    MODULE_REGISTRY.forEach((group, gi) => {
      ...
      const isCollapsed = welcomeCollapsed.has(group.group);

      const section = document.createElement('section');
      section.className = 'welcome-section' + (isCollapsed ? ' collapsed' : '');

      // Header doubles as the expand/collapse toggle (mirrors the sidebar).
      const header = document.createElement('button');
      header.className = 'welcome-section-header';
      header.id = groupAnchorId(group.group);
      header.setAttribute('aria-expanded', String(!isCollapsed));
      header.innerHTML = `
        <span class="welcome-section-heading">
          <span class="welcome-section-label">${group.icon} ${group.group}</span>
          <span class="welcome-section-count">${mods.length} ${mods.length === 1 ? 'module' : 'modules'}</span>
          <i class="bi bi-chevron-down welcome-section-chevron" aria-hidden="true"></i>
        </span>
        ${group.blurb ? `<span class="welcome-section-blurb">${group.blurb}</span>` : ''}
      `;
      header.addEventListener('click', () => {
        const nowCollapsed = section.classList.toggle('collapsed');
        header.setAttribute('aria-expanded', String(!nowCollapsed));
        if (nowCollapsed) welcomeCollapsed.add(group.group);
        else welcomeCollapsed.delete(group.group);
        saveWelcomeCollapsed();
      });

      const body = document.createElement('div');
      body.className = 'welcome-section-body';
      const bodyInner = document.createElement('div');
      bodyInner.className = 'welcome-section-body-inner';
      ...
      moduleEmpty.appendChild(section);
    });
  }
  ...
  function showWelcomeAndScrollTo(groupName) {
    ...
    if (anchor) {
      // Expand the section so the jumped-to topic is actually visible
      const section = anchor.closest('.welcome-section');
      if (section && section.classList.contains('collapsed')) {
        section.classList.remove('collapsed');
        anchor.setAttribute('aria-expanded', 'true');
        welcomeCollapsed.delete(groupName);
        saveWelcomeCollapsed();
      }
  ...
  function buildNavbarGroupLinks() {
    ...
    MODULE_REGISTRY.forEach((group, i) => {
      ...
      link.href = '#';
      link.textContent = group.navLabel || group.group;
