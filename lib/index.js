...
(function () {
  ...
  // Restore saved state, default to open if nothing saved yet
  var open = localStorage.getItem('sidebarOpen') !== 'false';

  // Apply the restored state immediately on load
  function applyState() {
  ...
  btn.addEventListener('click', function () {
    open = !open;
    localStorage.setItem('sidebarOpen', open); // <-- persist on every click
    applyState();
  });
}());
