...
// Basic shell syntax highlighting for example output
function highlightShell(text) {
  return escapeHtml(text)
    // Highlight comments (# ...)
    .replace(/^(\s*#[^\n]*)$/gm, '<span class="sh-comment">$1</span>')
    // Highlight prompt lines ($ or # at line start)
    .replace(/^(\s*[#$]\s)/gm, '<span class="sh-prompt">$1</span>');
}
...
function initApp() {
  ...
  let proseBlockIndex = 0; // track which prose block is first

  function renderProseBlock(block) {
    const isFirst = proseBlockIndex === 0;
    proseBlockIndex++;
    const div = document.createElement('div');
    div.className = 'module-prose mb-4' + (isFirst ? '' : ' collapsed');

    if (block.title) {
      // Collapsible card — first one expanded, rest collapsed
      const header = document.createElement('button');
      header.className = 'module-prose-header';
      header.setAttribute('aria-expanded', String(isFirst));
      header.setAttribute('aria-label', `Toggle: ${block.title}`);
      header.innerHTML = `
        <span class="module-prose-title">${block.title}</span>
        <i class="bi bi-chevron-down module-prose-chevron" aria-hidden="true"></i>
      `;
  ...
  function renderCommandsBlock(block, query) {
    ...
    cmds.forEach(cmd => {
      ...
      `).join('');
      const copyAllBtn = commandLines.length > 1 ? `
        <button class="btn btn-sm btn-outline-primary copy-btn mt-1"
          data-cmd="${fullCommand.replace(/"/g, '&quot;')}">📋 Copy all</button>
      ` : '';
        ...
        <div class="accordion-item border" id="item-${cmd.id}">
          ...
          <div id="${collapseId}" class="accordion-collapse collapse">
            <div class="accordion-body pt-0">
              ...
              <div class="mb-3">
                <div class="detail-label d-flex align-items-center justify-content-between">
                  <span>Example output</span>
                  <button class="btn btn-sm btn-outline-secondary copy-example-btn"
                    data-example-id="${cmd.id}">Copy</button>
                </div>
                <div class="example-box">${highlightShell(cmd.example)}</div>
              </div>
              <div class="why-box">
    ...
    accordion.addEventListener('click', async e => {
      ...
      const exampleBtn = e.target.closest('.copy-example-btn');
      if (exampleBtn) {
        await navigator.clipboard.writeText(exampleMap[exampleBtn.dataset.exampleId]);
        exampleBtn.textContent = '✓ Copied';
        setTimeout(() => (exampleBtn.textContent = 'Copy'), 2000);
      }
    });
  ...
  // ── Main render ─────────────────────────────────────────────────────────────
  function render() {
    proseBlockIndex = 0; // reset for each render cycle
