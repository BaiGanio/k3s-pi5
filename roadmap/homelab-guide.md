# 🏗️ Homelab DevOps Platform — Hardware & Tooling Guide

> **Hardware:** MacBook M1 32GB · Raspberry Pi 5 8GB + 500GB SSD · Asus X540S 4GB + 1TB HDD (worker node + MinIO storage + kiosk display)

> **How this fits:** This is the hardware-and-tooling companion to [the roadmap](./roadmap.md).
> The roadmap walks the [Rick & Morty reference app](../README.md) (Morty/Node frontend,
> Rick/.NET API, the garage/Postgres) through six curriculum stops. This guide covers the
> machines those stops run on, and the optional extras (object storage, a live ops screen) you
> can bolt on. Everything here is **self-hosted, free or free-tier, and runs on hardware you own.**

---

## Why Such Project?

Most DevOps job listings require hands-on experience with Kubernetes, IaC, CI/CD pipelines, and observability. Cloud certifications help, but a **live, running homelab** that you built yourself is a genuine differentiator. It is not just to pass a test — but to build something real, end to end, on your own hardware, for free. Certs are optional decoration; the working machine is the proof. See [the roadmap](./roadmap.md) for the staged path that produced it.
...
> **Built vs optional.** The curriculum's six stops are built on **k3s, Ansible, Docker,
> Jenkins (CI/CD), Nagios (monitoring), and AWS free-tier (cloud)** — these are done and
> documented as modules. The other tools below (Gitea/Woodpecker, Harbor, Vault, Terraform,
> Linkerd, the richer Prometheus/Grafana/Loki stack) are **optional extras and future stops** —
> all free and self-hostable, none required to walk the path. Each section flags which it is.

### 1. Kubernetes (k3s) — *built (Phase 2)*
...
### 2. Infrastructure as Code — Ansible *(built, Phase 1)* + Terraform *(optional)*
...
### 3. CI/CD Pipeline — Jenkins — *built (Phase 3)*
...
**Jenkins (what this project uses):** The CI/CD stop runs a self-hosted Jenkins controller with agents on dedicated Vagrant VMs. Pipelines are code (`Jenkinsfile`): build, test, containerize, and deploy the .NET and Node.js services backed by Postgres. Massive industry adoption, and seeing it run under your own roof is the fastest way to understand what managed pipelines do for you. Covered by the `jenkins-*` modules.

**Gitea + Woodpecker (optional, lighter alternative):** If you want a fully self-hosted Git + CI combo that's lighter than Jenkins, Gitea (a self-hosted GitHub) pairs with Woodpecker CI (pipeline-as-code via YAML). Not required — it's an alternative flavor of the same idea.
...
### 4. GitOps — ArgoCD — *optional / future stop*
...
### 5. Observability — Nagios *(built, Phase 3)* · Prometheus + Grafana + Loki *(optional, richer stack)*
...
**What this project built first: Nagios Core.** Check-based monitoring and alerting for the hosts, the .NET / Node APIs, Postgres, and containers — it flips a service `CRITICAL` and notifies you before users notice. A real on-prem/enterprise skill, covered by the `nagios-*` modules. The metrics-and-dashboards stack below is the complementary, also-free **next** step.
...
### 6. Container Registry — Harbor — *optional / future stop*
...
### 7. Secrets Management — Vault (HashiCorp) — *optional / future stop*
...
### 8. Object Storage — MinIO — *optional extra (runs on the Asus)*
...
### 9. Real-Time Dashboard Kiosk — Asus as a Live Ops Screen — *optional extra*
...
| Tool | Category | Status | Why It's Relevant |
|---|---|---|---|
| Vagrant | VMs / Provisioning | ✅ Built (Ph.1) | Reproducible Linux VMs from one file |
| Ansible | Config Mgmt | ✅ Built (Ph.1) | Agentless, idempotent node configuration |
| Docker | Containers | ✅ Built (Ph.1) | Package apps as portable images |
| k3s | Orchestration | ✅ Built (Ph.2) | Industry-standard Kubernetes, lightweight for homelab |
| Cloudflare Tunnel | Networking | ✅ Built (Ph.2) | Free public HTTPS with no static IP |
| Jenkins | CI/CD | ✅ Built (Ph.3) | Self-hosted pipelines: build, test, deploy |
| Nagios Core | Monitoring | ✅ Built (Ph.3) | Check-based monitoring + alerting |
| AWS (free tier) | Cloud | ✅ Built (Ph.4) | EC2, IAM, ECS — cloud reach, cost-aware |
| Helm | Package Manager | 🔭 Optional | Kubernetes app packaging standard |
| ArgoCD / Flux | GitOps | 🔭 Optional | Reconcile-from-Git CD discipline |
| Terraform | IaC | 🔭 Optional | Declarative cloud provisioning |
| Gitea + Woodpecker | Source / CI | 🔭 Optional | Lighter self-hosted Git + CI alternative |
| Harbor | Registry | 🔭 Optional | Enterprise container registry |
| Prometheus / Grafana / Loki | Metrics / Logs | 🔭 Optional | Richer metrics-and-dashboards stack |
| Vault | Secrets | 🔭 Optional | Enterprise secrets management |
| KEDA | Autoscaling | 🔭 Optional | Event-driven pod scaling |
| Linkerd | Service Mesh | 🔭 Optional | mTLS, traffic management, observability |
| MinIO | Object Storage | ➕ Extra | S3-compatible self-hosted blob storage; same SDK as AWS S3 |
| Nginx | Reverse Proxy / CDN | ➕ Extra | Caching layer in front of MinIO; mirrors CDN-backed storage |
